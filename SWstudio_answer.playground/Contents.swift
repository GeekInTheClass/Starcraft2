...
let sum = nums.reduce(0, { (v1: Int, v2: Int) -> (Int) in
    return v1 + v2
})
...
var result: [String] = list2.sorted()
print(result)
...
let minusNum = a.filter({ (value: Int) -> Bool in return (value < 0) })
count = minusNum.count
print(count)
